...
def strategy_sma50vs200(item, df=pd.DataFrame()):
    lastPosition=''
    lastIndex = ''
    lastPrice = ''
    ...
    for index, row in df.iterrows():
        if row['SMA50'] > row['SMA200'] and usd > 10:
            share = usd / row['Close']
            usd = 0
        ...
        if row['SMA50'] < row['SMA200'] and share > 1:
            usd = share * row['Close']
            share = 0
...
def strategy_ichimoku(item, df=pd.DataFrame()):
    lastPosition=''
    lastIndex = ''
    lastPrice = 0
    ...
    for index, row in df.iterrows():
        if row['Close'] > row['SSA'] and row['Close'] > row['SSB'] and usd > 10:
            ...
            elif row['TENKAN'] < row['KIJUN'] and share > 1 : 
        ...
        if ((row['Close'] < row['SSA']) and (row['Close'] < row['SSB'])) and share > 1:
            usd = share * df['Close'][index]
            lastIndex = index
